...
/// Protocol for any LLM backend that can complete prompts.
...
/// Logical tiers of LLM usage within Oracle OS.
public enum LLMModelTier: String, Sendable, Equatable {
    case planning
...
// MARK: - LLMClientDiagnostics

/// Aggregate diagnostics about LLM client usage.
...
/// Dispatches LLM requests to registered providers by model tier.
public final class LLMClient: @unchecked Sendable {
    ...
    public var diagnostics: LLMClientDiagnostics {
        lock.lock()
        defer { lock.unlock() }
        return LLMClientDiagnostics(requestCount: requestCount, totalTokens: totalTokens)
    }
}
