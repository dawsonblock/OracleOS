...
public final class ProjectMemoryIndexer: @unchecked Sendable {
    ...
    public func rebuild(from rootURLs: [URL]) {
        ...
        for rootURL in rootURLs {
            guard fileManager.fileExists(atPath: rootURL.path),
                  let enumerator = fileManager.enumerator(at: rootURL, includingPropertiesForKeys: [.isRegularFileKey], options: [.skipsHiddenFiles]) else {
                continue
            }
            for case let fileURL as URL in enumerator where fileURL.pathExtension == "md" {
                guard let record = Self.parseRecord(fileURL: fileURL) else { continue }
                upsert(record)
            }
        }
    }
    ...
    public func upsert(_ record: ProjectMemoryRecord) {
        ...
        withStatement(sql) { statement in
            ...
            bind(record.summary, to: 11, in: statement)
    ...
    public func query(text: String, modules: [String] = [], kinds: [ProjectMemoryKind] = [], limit: Int = 10) -> [ProjectMemoryRef] {
        var clauses: [String] = []
        if !text.isEmpty { clauses.append("(title LIKE ? OR summary LIKE ? OR body LIKE ?)") }
        if !modules.isEmpty { clauses.append("(" + modules.map { _ in "affected_modules LIKE ?" }.joined(separator: " OR ") + ")") }
        if !kinds.isEmpty { clauses.append("(" + kinds.map { _ in "kind = ?" }.joined(separator: " OR ") + ")") }
        ...
        if !modules.isEmpty { params.append(contentsOf: modules.map { "%\($0)%" }) }
        if !kinds.isEmpty { params.append(contentsOf: kinds.map(\.rawValue)) }
        ...
        while sqlite3_step(statement) == SQLITE_ROW {
            ...
            records.append(ProjectMemoryRef(
                ...
                affectedModules: splitPM(columnText(statement, index: 7)),
                evidenceRefs: splitPM(columnText(statement, index: 8)),
                sourceTraceIDs: splitPM(columnText(statement, index: 9))
            ))
        }
        return records
    }
    ...
    public static func parseRecord(fileURL: URL) -> ProjectMemoryRecord? {
        ...
        guard let firstHeading = lines.first(where: { $0.hasPrefix("# ") }) else { return nil }
        ...
        return ProjectMemoryRecord(
            ...
            path: fileURL.path,
            affectedModules: splitPM(metadata["affected_modules"]),
            evidenceRefs: splitPM(metadata["evidence_refs"]),
            sourceTraceIDs: splitPM(metadata["source_trace_ids"]),
            createdAt: createdAt,
            updatedAt: updatedAt
        )
    }
    ...
    private func execute(_ sql: String) throws {
        ...
        guard sqlite3_exec(db, sql, nil, nil, &errorMessage) == SQLITE_OK else {
            ...
            throw NSError(domain: "ProjectMemoryIndexer", code: 2, userInfo: [NSLocalizedDescriptionKey: message])
        }
    }
...
private func splitPM(_ value: String?) -> [String] {
