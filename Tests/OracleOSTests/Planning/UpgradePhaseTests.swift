...
struct UpgradePhaseTests {
    ...
    func graphNavigatorBeamWidth() {
        ...
        let paths = navigator.expand(from: root.id, in: graph, scorer: scorer, goal: Goal(description: ""), allowedFamilies: [])
    ...
    func workflowMatcherNoMatch() {
        ...
        let matches = matcher.match(currentState: .repoLoaded, workflowIndex: index, selectedStrategy: SelectedStrategy(kind: .directExecution, confidence: 1.0, rationale: "", allowedOperatorFamilies: []))
