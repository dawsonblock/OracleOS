...
struct TaskGraphTests {
    ...
    private func makeRepoSnapshot() -> RepositorySnapshot {
        RepositorySnapshot(
            ...
            buildTool: .swiftPackage, files: [], symbolGraph: SymbolGraph(), dependencyGraph: DependencyGraph(), testGraph: TestGraph(), activeBranch: "main", isGitDirty: false)
    }
    ...
    func stateAbstractorTestsRunning() {
        let abstractor = StateAbstractor()
        _ = makePlanningState(id: "code|test", taskPhase: "test_running")
        let repo = makeRepoSnapshot()
        let ws = WorldState(
    ...
    func stateAbstractorModal() {
        let abstractor = StateAbstractor()
        _ = makePlanningState(id: "modal", modalClass: "alert")
        let ws = WorldState(
            observationHash: "hash",
            planningState: ps,
    ...
    func stateAbstractorPermission() {
        let abstractor = StateAbstractor()
        _ = makePlanningState(id: "perm", modalClass: "permission_dialog")
        let ws = WorldState(
            observationHash: "hash",
            planningState: ps,
    ...
    func graphNavigatorExpand() {
        ...
        let scorer = GraphScorer()
        let paths = navigator.expand(from: nodeA.id, in: graph, scorer: scorer, allowedFamilies: [])
    ...
    func graphNavigatorBestEdge() {
        ...
        let scorer = GraphScorer()
        let best = navigator.bestNextEdge(from: nodeA.id, in: graph, scorer: scorer, allowedFamilies: [])
    ...
    func taskGraphStoreUpdateCurrent() {
        ...
        _ = makePlanningState(id: "state-1", appID: "Chrome", taskPhase: "browse")
        let ws = WorldState(
    ...
    func taskGraphStoreEdgeLifecycle() {
        ...
        if let edgeID = edge?.id {
            ...
            #expect(resultNode != nil /* removed constraint always true */ /* removed constraint always true */)
    ...
    func taskGraphStoreRecoveryEdges() {
        let store = TaskGraphStore()
        _ = makePlanningState(id: "s1", appID: "App", taskPhase: "browse")
        let ws = WorldState(
            observationHash: "h1",
            planningState: ps,
    ...
    func fullTaskGraphCycle() {
        ...
        let paths = navigator.expand(
            from: startNode.id,
            in: store.graph,
            scorer: scorer, goal: Goal(description: "fix failing tests"), allowedFamilies: []
        )
