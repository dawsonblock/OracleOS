...
struct LLMAdvisorTests {
    ...
    func repairAdvisorFallsBackWithoutProvider() async {
        ...
        let advice = await advisor.advise(
            ...
            memoryInfluence: MemoryInfluence(preferredFixPath: "Sources/Calculator.swift"), selectedStrategy: SelectedStrategy(kind: .directExecution, confidence: 1.0, rationale: "", allowedOperatorFamilies: [])
        )
    ...
    func repairAdvisorUsesLLM() async {
        ...
        let advice = await advisor.advise(
            ...
            memoryInfluence: MemoryInfluence(), selectedStrategy: SelectedStrategy(kind: .directExecution, confidence: 1.0, rationale: "", allowedOperatorFamilies: [])
        )
    ...
    func targetResolverReturnsEmptyWithoutProvider() async {
        ...
        let result = await resolver.resolve(
            ...
            visibleElements: ["Submit button", "Username field"], selectedStrategy: SelectedStrategy(kind: .directExecution, confidence: 1.0, rationale: "", allowedOperatorFamilies: [])
        )
    ...
    func recoveryAdvisorPrefersMemoryStrategy() async {
        ...
        let plan = await advisor.advise(
            ...
            recentActions: ["click"],
            memoryInfluence: MemoryInfluence(preferredRecoveryStrategy: "refocus_application"), selectedStrategy: SelectedStrategy(kind: .directExecution, confidence: 1.0, rationale: "", allowedOperatorFamilies: [])
        )
