...
struct SystemIntelligenceUpgradeTests {
    ...
    func plannerDecisionHierarchyPrefersWorkflow() {
        ...
        let decision = planner.nextStep(
            ...
            graphStore: GraphStore(databaseURL: makeTempGraphURL()), selectedStrategy: SelectedStrategy(kind: .directExecution, confidence: 1.0, rationale: "", allowedOperatorFamilies: [])
        )
