...
// MARK: - ReasoningPlanningState

/// A flat state representation used by the reasoning layer for operator
/// precondition evaluation, projection, and plan generation.
public struct ReasoningPlanningState: Sendable, Hashable {
    ...
    // MARK: - Designated initialiser (from full context)
    ...
    public init(
        ...
        let testsObserved = loweredGoal.contains("test")
            ...
            || worldState.lastAction?.type == "run_tests"
            || worldState.lastAction?.type == "test"
        ...
        self.failingTests = testsObserved ? 1 : nil  // simplified: always 1 if observed
    ...
    // MARK: - Convenience initialiser (for testing / direct creation)

    public init(
        agentKind: AgentKind = .code,
        goalDescription: String = "",
        planningStateID: PlanningStateID = PlanningStateID(rawValue: "default"),
        ...
        self.visibleTargets = visibleTargets
        self.repoOpen = repoOpen
        self.repoDirty = repoDirty
        self.buildSucceeded = buildSucceeded
        self.failingTests = failingTests
        self.testsObserved = testsObserved
        self.patchApplied = patchApplied
        self.modalPresent = modalPresent
        self.preferredWorkspacePath = preferredWorkspacePath
        self.candidateWorkspacePaths = candidateWorkspacePaths
    ...
    private static func modalPresent(worldState: WorldState) -> Bool {
        if worldState.planningState.modalClass != nil { return true }
    ...
    private static func candidateWorkspacePaths(
        ...
        var paths: [String] = []
        if let preferredPath { paths.append(preferredPath) }
