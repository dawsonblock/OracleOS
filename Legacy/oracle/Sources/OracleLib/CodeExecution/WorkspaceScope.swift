...
public enum WorkspaceScopeError: Error, LocalizedError, Sendable, Equatable {
    ...
    public var errorDescription: String? {
        ...
        case .outsideWorkspace(let path):
            return "Path is outside workspace scope: \(path)"
        case .missingPath(let path):
            return "Missing workspace path: \(path)"
        }
    }
}
...
public struct WorkspaceScope: Sendable, Equatable {
    ...
    public func resolve(relativePath: String?) throws -> URL? {
        ...
        guard !relativePath.isEmpty else { throw WorkspaceScopeError.missingPath(relativePath) }
