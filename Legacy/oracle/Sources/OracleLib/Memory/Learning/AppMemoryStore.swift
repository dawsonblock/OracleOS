...
/// In-memory store for all app-level learning: controls, failures, strategies,
/// protected operations, approvals, and code memory.
public final class AppMemoryStore {
    ...
    public func preferredRecoveryStrategy(app: String) -> String? {
        latestSuccessfulStrategy(app: app)?.strategy
    }

    // MARK: - Protected operations & approvals
    ...
    public func rankingBias(label: String?, app: String?) -> Double {
        ...
        guard control.successCount >= 3, total > 0 else { return 0 }
    ...
    public func fixPatterns(for errorSignature: String) -> [FixPattern] {
        codeMemory.fixPatterns.values.filter { $0.errorSignature == errorSignature }
    }
