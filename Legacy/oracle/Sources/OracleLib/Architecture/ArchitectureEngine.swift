...
// MARK: - ArchitectureEngine

/// Orchestrates all architecture-governance subsystems for review and patch analysis.
public final class ArchitectureEngine: @unchecked Sendable {
    ...
    /// Performs a full architecture review for a proposed change.
    public func review(
    ...
    // MARK: - Candidate Patch Review

    /// Reviews a single candidate patch for heuristic and governance violations.
    public func reviewCandidatePatch(
        ...
        return ArchitectureReview(
            ...
            affectedModules: Array(Set(
                baseReview.affectedModules + heuristicFindings.flatMap(\.affectedModules)
            )).sorted(),
            findings: findings,
            refactorProposal: proposal,
            riskScore: riskScore,
    ...
    private func candidateHeuristicFindings(
        ...
        // Test-only repair path
        if path.hasPrefix("Tests/"),
           loweredGoal.contains("fix") || loweredGoal.contains("repair")
               || loweredGoal.contains("failing") || loweredGoal.contains("build")
               || loweredGoal.contains("test") {
        ...
        // Public interface change during non-refactor task
        ...
        // Planner/execution boundary drift
