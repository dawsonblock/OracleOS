...
// MARK: - InvariantChecker

/// Checks a proposed change against Oracle's architectural invariants,
/// producing a `GovernanceReport` with any violations found.
public struct InvariantChecker: Sendable {
    ...
    public func report(
        ...
        // G3: Planning/execution boundary drift
        ...
        // G1: Loop orchestration drift
        if moduleSet.contains("Agent/Loop"),
           !moduleSet.intersection(["Experiments", "Architecture", "Core/Ranking", "Agent/Planning"]).isEmpty {
            violations.append(GovernanceViolation(
        ...
        // G1: Execution truth path bypass risk
        ...
        // G1: Policy/execution boundary drift (advisory)
        if moduleSet.contains("Runtime"), moduleSet.contains("Core/Execution"),
           loweredGoal.contains("policy") {
            violations.append(GovernanceViolation(
                ruleID: .executionTruthPath,
                severity: .advisory,
        ...
        // G3: Target resolution bypass / planner boundary drift / skill-ranking integrity
        ...
        if touchesTargetBearingSkill, !touchesRankingPath {
            violations.append(GovernanceViolation(
                ...
                evidence: candidatePaths
                  ...
                  !moduleSet.intersection(["Core/Ranking", "CodeExecution", "Core/World"]).isEmpty {
            violations.append(GovernanceViolation(
                ruleID: .hierarchicalPlanning,
                severity: .hardFail,
        ...
        } else if moduleSet.contains("Agent/Skills"), moduleSet.contains("Core/Ranking"),
                  loweredGoal.contains("click") {
            violations.append(GovernanceViolation(
                ruleID: .hierarchicalPlanning,
                severity: .advisory,
        ...
        // G2: Trust-tier promotion drift
        if (moduleSet.contains("Graph") && moduleSet.contains("Experiments"))
            || (moduleSet.contains("Graph") && moduleSet.contains("Agent/Recovery")) {
            violations.append(GovernanceViolation(
        ...
        // G4: Recovery path drift (advisory)
        ...
        // G4: Recovery tagging must remain explicit
        ...
        // G5: Architecture growth without eval coverage
        if ChangeImpactAnalyzer().shouldReview(goalDescription: goalDescription, candidatePaths: candidatePaths),
           !hasTests {
            violations.append(GovernanceViolation(
