...
// MARK: - DependencyAnalyzer

public struct DependencyAnalyzer: Sendable {
    public init() {}

    /// Finds dependency cycles in a module graph using iterative DFS.
    /// Returns each cycle as an ordered array of module names.
    public func findCycles(in moduleGraph: ArchitectureModuleGraph) -> [[String]] {
        ...
        for node in moduleGraph.modules.keys.sorted() where !visited.contains(node) {
            dfs(node)
        }
    ...
    /// Returns an `ArchitectureFinding` for each dependency cycle detected.
