...
public final class PlanGenerator: @unchecked Sendable {
    ...
    private func familyBacked(
        ...
        // Construct ReasoningPlanningState
        let memoryRouter = MemoryRouter(memoryStore: memoryStore)
        let memoryInfluence = memoryRouter.influence(for: MemoryQueryContext(taskContext: taskContext, worldState: worldState))
        let reasoningState = ReasoningPlanningState(taskContext: taskContext, worldState: worldState, memoryInfluence: memoryInfluence)
        ...
        return [
            PlanCandidate(
                operators: [op],
                projectedState: reasoningState,
                reasons: decision.notes + ["family-backed decision from \(decision.plannerFamily)"],
                sourceType: mapPlannerSource(decision.source)
            )
        ]
    }

    private func mapPlannerSource(_ source: PlannerSource) -> PlanSourceType {
        switch source {
